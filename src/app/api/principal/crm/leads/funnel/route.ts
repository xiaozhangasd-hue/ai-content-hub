...
/**
 * 获取转化漏斗数据
 * GET /api/principal/crm/leads/funnel
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const funnel = [
      { name: "咨询", value: newCount + contactedCount },
      { name: "邀约", value: invitedCount },
      { name: "试听", value: trialCount },
      { name: "报名", value: signedCount },
    ];
    ...
    const needFollowUp = await prisma.customer.findMany({
      where: {
        merchantId,
        status: { in: ["new", "contacted", "invited"] },
        OR: [
